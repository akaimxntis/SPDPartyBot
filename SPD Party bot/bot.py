...
async def on_ready():
    ...
    try:
        # Limpa comandos antigos registrados por servidor.
        # Isso remove duplicados como /setup-party-hub e /party-config-panel
        # que ficaram de versões anteriores, sem apagar os comandos globais atuais.
        for guild in bot.guilds:
            bot.tree.clear_commands(guild=discord.Object(id=guild.id))
            await bot.tree.sync(guild=discord.Object(id=guild.id))
            print(f"Comandos antigos limpos em: {guild.name}")
...
I18N = {
    "pt-BR": {
        ...
        "config_pending": "⚠️ Configuração pendente",
        "config_pending_desc": "A staff ainda precisa configurar o canal dos convites.",
        "hub_footer": "Use /party-config-panel para configurar o bot",
        "create_party": "Criar Party",
        "list_parties": "Ver Parties",
        "my_parties": "Minhas Parties",
        "help": "Ajuda",
    ...
    "en-US": {
        ...
        "config_pending": "⚠️ Setup pending",
        "config_pending_desc": "Staff still needs to configure the invite channel.",
        "hub_footer": "Use /party-config-panel to configure the bot",
        "create_party": "Create Party",
        "list_parties": "View Parties",
        "my_parties": "My Parties",
        "help": "Help",
        "use_inside_server": "Use this inside a server.",
        "no_perm_create": "You do not have permission to create parties.\nRequired role: {required}.",
        "no_party_channel": "The invite channel has not been configured yet.\nStaff needs to use `/party-config-panel`.",
        "no_open_parties": "There are no open parties right now.",
        "not_in_party": "You are not in any open party.",
        "help_text": (
            "**How it works:**\n\n"
...
async def setup_party_hub(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.response.send_message(tr(guild_id, "party_create_error", error=error), ephemeral=True)
        return
...
async def party_config_panel(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(embed=await make_config_embed(interaction.guild), view=ConfigPanelView(interaction.guild.id), ephemeral=True)
...
async def party_clean_closed(interaction: discord.Interaction):
    ...
    save_parties()
    await interaction.response.send_message(tr(guild_id, "clean_done", count=before - len(parties)), ephemeral=True)
