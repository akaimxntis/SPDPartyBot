...
PARTIES_PATH = BASE_DIR / "parties.json"
...
class ConfigPanelView(discord.ui.View):
    ...
    @discord.ui.button(label="ID Cargo Host", emoji="🔢", style=discord.ButtonStyle.secondary, row=2)
    ...
    @discord.ui.button(label="Todos podem criar", emoji="👥", style=discord.ButtonStyle.secondary, row=3)
    ...
    @discord.ui.button(label="Ping @everyone", emoji="📣", style=discord.ButtonStyle.secondary, row=3)
    ...
    @discord.ui.button(label="Cor do Embed", emoji="🎨", style=discord.ButtonStyle.secondary, row=4)
    ...
    @discord.ui.button(label="Atualizar Painel", emoji="🔄", style=discord.ButtonStyle.success, row=4)
...
def load_json(path: Path, default):
    ...
    if isinstance(default, dict) and isinstance(data, dict):
        ...
        if "hub_channel_id" in data:
            data.pop("hub_channel_id", None)
            changed = True
...
def refresh_config_values():
    global TOKEN
    global GUILD_ID
    global PARTY_CHANNEL_ID
    ...
    PARTY_CHANNEL_ID = int(config.get("party_channel_id", 0))
    LOG_CHANNEL_ID = int(config.get("log_channel_id", 0))
    HOST_ROLE_ID = int(config.get("host_role_id", 0))
...
def config_status_text() -> str:
    ...
    return (
        f"**Canal de convites:** {channel_text(PARTY_CHANNEL_ID)}\n"
        f"**Canal de logs:** {channel_text(LOG_CHANNEL_ID)}\n"
        f"**Cargo Host:** {role_text(HOST_ROLE_ID)}\n"
        f"**Cargo Staff:** {role_text(STAFF_ROLE_ID)}\n"
        f"**Cor do embed:** `{EMBED_COLOR}`"
    )


def make_hub_embed() -> discord.Embed:
    embed = discord.Embed(
        title="🎮 Hub de Parties — SPD",
        description=(
            "Crie e acompanhe convites de party pelo painel abaixo.\n\n"
...
def make_party_embed(party_id: str) -> discord.Embed:
    ...
    embed = discord.Embed(
        title=title,
        description=data.get("description") or None,
        color=EMBED_COLOR if status == "open" else 0x555555,
    )
    ...
    if queue:
        embed.add_field(
            name=f"📋 Fila ({len(queue)})",
            value=mention_list(queue),
            inline=False,
        )
    ...
    embed.set_footer(
        text=f"Created by {data.get('host_name', 'unknown')} • Party ID: {party_id}"
    )
...
async def update_party_message(party_id: str):
    ...
    channel = await fetch_text_channel(int(data.get("channel_id", 0)))
...
class HubView(discord.ui.View):
    ...
    async def create_party(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not can_create_party(member):
            await interaction.response.send_message(
                f"Você não tem permissão para criar parties.\n"
                f"Cargo necessário: <@&{HOST_ROLE_ID}>.",
                ephemeral=True,
            )
            return
    ...
    async def list_parties(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not open_parties:
            await interaction.response.send_message(
                "Não tem nenhuma party aberta no momento.",
                ephemeral=True,
            )
            return
    ...
    @discord.ui.button(
        label="Minhas Parties",
        emoji="👤",
        style=discord.ButtonStyle.secondary,
        custom_id="hub:my_parties",
    )
    async def my_parties(self, interaction: discord.Interaction, button: discord.ui.Button):
        user_id = interaction.user.id
        found = []
        ...
        if not found:
            await interaction.response.send_message(
                "Você não está em nenhuma party aberta.",
                ephemeral=True,
            )
            return
    ...
    async def help_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        text = (
            ...
            "5. Se **Vou** lotar, quem clicar em ✅ vai para a fila.\n"
            "6. O host ou a staff usa **⚙️ Gerenciar** para editar ou encerrar.\n\n"
            f"Cargo para criar party: <@&{HOST_ROLE_ID}>."
        )
...
class CreatePartyModal(discord.ui.Modal, title="Criar Party"):
    title_input = discord.ui.TextInput(
        label="Título da party",
        placeholder="Ex: Lego Party - 4 Jogadores",
        max_length=100,
        required=True,
    )
    ...
    time = discord.ui.TextInput(
        label="Horário",
        placeholder="Ex: Hoje 18:30 - 20:30",
        max_length=120,
        required=False,
    )
    ...
    description = discord.ui.TextInput(
        label="Descrição",
        placeholder="Ex: Bora jogar LEGO Party!",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not interaction.guild:
            await interaction.followup.send(
                "Esse formulário só funciona dentro de servidor.",
                ephemeral=True,
            )
            return
        ...
        if not can_create_party(member):
            await interaction.followup.send(
                "Você não tem permissão para criar parties.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            await interaction.followup.send(
                "O campo **Vagas** precisa ser um número.",
                ephemeral=True,
            )
            return
        ...
        if capacity_value < 1 or capacity_value > 25:
            await interaction.followup.send(
                "As vagas precisam ficar entre **1** e **25**.",
                ephemeral=True,
            )
            return
        ...
        channel = await fetch_text_channel(PARTY_CHANNEL_ID)
        ...
        try:
            message = await channel.send(
                content="@everyone",
                embed=make_party_embed(party_id),
                view=PartyView(),
            ...
            await interaction.followup.send(
                "Achei o canal de convites, mas não tenho permissão para enviar mensagem nele.",
                ephemeral=True,
            )
            return
        except discord.HTTPException as error:
        ...
        await interaction.followup.send(
            f"Party criada com sucesso em {channel.mention}: {message.jump_url}",
            ephemeral=True,
        )

        await log_action(
            interaction.guild,
            f"🎮 <@{member.id}> criou a party **{parties[party_id]['title']}** "
            f"`{party_id}` em {channel.mention}.",
        )
...
class EditPartyModal(discord.ui.Modal, title="Editar Party"):
    def __init__(self, party_id: str):
        ...
        self.capacity = discord.ui.TextInput(
            label="Vagas",
            default=str(data.get("capacity", 4)),
            max_length=2,
            required=True,
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not isinstance(member, discord.Member) or not can_manage_party(member, data):
            await interaction.followup.send(
                "Só o host ou a staff pode editar essa party.",
                ephemeral=True,
            )
            return
        ...
        await interaction.followup.send(
            "Party editada com sucesso.",
            ephemeral=True,
        )

        await log_action(
            interaction.guild,
            f"✏️ <@{member.id}> editou a party **{data['title']}** `{self.party_id}`.",
        )
...
class ManagePartyView(discord.ui.View):
    ...
    async def close_party(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not isinstance(member, discord.Member) or not can_manage_party(member, data):
            await interaction.response.send_message(
                "Só o host ou a staff pode encerrar essa party.",
                ephemeral=True,
            )
            return
        ...
        await log_action(
            interaction.guild,
            f"🔒 <@{member.id}> encerrou a party **{data['title']}** `{self.party_id}`.",
        )
...
class PartyView(discord.ui.View):
    ...
    async def get_party(self, interaction: discord.Interaction) -> Optional[str]:
        if not interaction.message:
            await interaction.response.send_message(
                "Não consegui identificar essa party.",
                ephemeral=True,
            )
            return None
        ...
        if not party_id or party_id not in parties:
            await interaction.response.send_message(
                "Essa party não existe mais no banco de dados.",
                ephemeral=True,
            )
            return None
    ...
    async def tentative_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await log_action(
            interaction.guild,
            f"❔ <@{user_id}> marcou Talvez na party **{data['title']}** `{party_id}`.",
        )
    ...
    async def declined_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await log_action(
            interaction.guild,
            f"❌ <@{user_id}> marcou Não vou na party **{data['title']}** `{party_id}`.",
        )
    ...
    async def manage_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not isinstance(member, discord.Member) or not can_manage_party(member, data):
            await interaction.response.send_message(
                "Só o host ou a staff pode gerenciar essa party.",
                ephemeral=True,
            )
            return
...
class PartyChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        channel = self.values[0]
        config["party_channel_id"] = channel.id
        save_config()
        ...
        await log_action(
            interaction.guild,
            f"⚙️ <@{interaction.user.id}> definiu o canal de convites como {channel.mention}.",
        )
...
class LogChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await log_action(
            interaction.guild,
            f"⚙️ <@{interaction.user.id}> definiu o canal de logs como {channel.mention}.",
        )
...
class HostRoleSelect(discord.ui.RoleSelect):
    def __init__(self):
        super().__init__(
            placeholder="Escolha o cargo que pode criar parties",
            min_values=1,
            max_values=1,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await log_action(
            interaction.guild,
            f"⚙️ <@{interaction.user.id}> definiu o cargo Host como {role.mention}.",
        )
...
class StaffRoleSelect(discord.ui.RoleSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await log_action(
            interaction.guild,
            f"⚙️ <@{interaction.user.id}> definiu o cargo Staff como {role.mention}.",
        )


class ConfigColorModal(discord.ui.Modal, title="Alterar cor do embed"):
    color_value = discord.ui.TextInput(
        label="Cor decimal",
        placeholder="Ex: 16766720",
        max_length=8,
        required=True,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(
                "A cor precisa ser um número decimal.",
                ephemeral=True,
            )
            return
        ...
        if value < 0 or value > 16777215:
            await interaction.followup.send(
                "A cor precisa estar entre `0` e `16777215`.",
                ephemeral=True,
            )
            return
        ...
        await interaction.followup.send(
            f"Cor alterada para `{value}`.",
            ephemeral=True,
        )

        await log_action(
            interaction.guild,
            f"⚙️ <@{interaction.user.id}> alterou a cor do embed para `{value}`.",
        )
...
def make_config_embed() -> discord.Embed:
    embed = discord.Embed(
        title="⚙️ Configuração do Party Bot",
        description=(
            config_status_text()
...
class ConfigPanelView(discord.ui.View):
    ...
    async def set_log_channel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            content="Escolha o canal de logs:",
            embed=None,
            view=SelectOnlyView(LogChannelSelect()),
        )
    ...
    async def set_host_role(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            content="Escolha o cargo que pode criar parties:",
            embed=None,
            view=SelectOnlyView(HostRoleSelect()),
        )
    ...
    async def set_staff_role(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            content="Escolha o cargo da staff:",
            embed=None,
            view=SelectOnlyView(StaffRoleSelect()),
        )
...
@bot.tree.command(
    name="setup-party-hub",
    description="Cria o Hub de Parties no canal onde o comando foi usado.",
    guild=discord.Object(id=GUILD_ID),
)
async def setup_party_hub(interaction: discord.Interaction):
    member = interaction.user
    ...
    if not is_staff(member):
        await interaction.response.send_message(
            "Você precisa ser staff ou ter **Gerenciar servidor** para criar o Hub.",
            ephemeral=True,
        )
        return
    ...
    if not isinstance(interaction.channel, discord.TextChannel):
        await interaction.response.send_message(
            "Use esse comando em um canal de texto normal.",
            ephemeral=True,
        )
        return

    try:
        message = await interaction.channel.send(embed=make_hub_embed(), view=HubView())
    except discord.Forbidden:
        await interaction.response.send_message(
        ...
        await interaction.response.send_message(
            f"Erro ao criar o Hub: `{error}`",
            ephemeral=True,
        )
        return

    await interaction.response.send_message(
        f"Hub criado nesse canal: {message.jump_url}",
        ephemeral=True,
    )

    await log_action(
        interaction.guild,
        f"🧩 <@{member.id}> criou o Hub de Parties em {interaction.channel.mention}.",
    )


@bot.tree.command(
    name="party-config-panel",
    description="Abre o painel de configuração do Party Bot.",
    guild=discord.Object(id=GUILD_ID),
)
async def party_config_panel(interaction: discord.Interaction):
    member = interaction.user
    ...
    if not is_staff(member):
        await interaction.response.send_message(
            "Você precisa ser staff ou ter **Gerenciar servidor** para configurar o bot.",
            ephemeral=True,
        )
        return
...
@bot.tree.command(
    name="party-list",
    description="Lista parties abertas.",
    guild=discord.Object(id=GUILD_ID),
)
async def party_list(interaction: discord.Interaction):
    open_parties = [
...
@bot.tree.command(
    name="party-clean-closed",
    description="Remove do banco as parties encerradas.",
    guild=discord.Object(id=GUILD_ID),
)
async def party_clean_closed(interaction: discord.Interaction):
    member = interaction.user
    ...
    if not isinstance(member, discord.Member) or not is_staff(member):
        await interaction.response.send_message(
            "Só a staff pode limpar parties encerradas.",
            ephemeral=True,
        )
        return
    ...
    await interaction.response.send_message(
        f"Limpeza concluída. Removidas: **{before - len(parties)}** parties encerradas.",
        ephemeral=True,
    )


if not TOKEN or TOKEN in ("COLE_SEU_TOKEN_AQUI", "SEU_TOKEN", "COLOQUE_SEU_TOKEN_AQUI"):
    raise RuntimeError("Coloque o token real do bot no config.json.")

if not GUILD_ID:
    raise RuntimeError("Coloque o guild_id real do seu servidor no config.json.")
